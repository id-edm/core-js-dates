...
function getTime(date) {
  ...
  const hours = formatTimeUnit(newDate.getHours());
  const minutes = formatTimeUnit(newDate.getMinutes());
  const seconds = formatTimeUnit(newDate.getSeconds());
...
function getDayName(date) {
  ...
  const newDate = new Date(date);
  return namesDay[newDate.getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  let daysUntilNextFriday = 0;

  if (dayOfWeek < 5) {
    daysUntilNextFriday = 5 - dayOfWeek;
  } else if (dayOfWeek === 5) {
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const days = Math.floor(difference / millisecondsInDay) + 1;
...
function isDateInPeriod(date, period) {
  ...
  const range = targetDate >= start && targetDate <= end;
  return range;
}
...
function getCountWeekendsInMonth(month, year) {
  let weekendCount = 0;
  const monthAdjusted = month - 1;
  const daysInMonth = new Date(year, month, 0).getDate();

  for (let day = 1; day <= daysInMonth; day += 1) {
    const currentDay = new Date(year, monthAdjusted, day).getDay();
...
function getWeekNumberByDate(date) {
  ...
  const isoDayOfWeek = (currentDate.getDay() + 6) % 7;

  currentDate.setDate(currentDate.getDate() - isoDayOfWeek + 3);
  ...
  const diffInDays = (currentDate - firstThursday) / millisecondsInDay;

  return Math.trunc(diffInDays / 7) + 1;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let currentDate = new Date(startDate); currentDate <= endDate; ) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; i += 1) {
      schedule.push(formatScheduleDate(currentDate));
